...
def non_decrease_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            return [[]] # in the base case return the nested empty list
        elif s[0] < prev:
            return subseq_helper(s[1:], prev) # return valid substrings that include prev
        else:
            a = subseq_helper(s[1:], s[0]) # return all valid substrings without s[0]
            b = subseq_helper(s[1:], prev) # return valid substrings from previous substring
            return insert_into_all(s[0], a) + b # add s[0] to all valid substrings without s[0]
