...
def deep_map_mut(func, lnk):
    ...
    """
    pos = lnk
    while pos.rest != Link.empty:
        if isinstance(pos.first, Link):
            deep_map_mut(func, pos.first)
        else:
            pos.first = func(pos.first)
        pos = pos.rest
    if isinstance(pos.first, Link):
        deep_map_mut(func, pos.first)
    else:
...
def two_list(vals, counts):
    ...
    for i in range(len(vals)):
        ...
        while j < counts[i]:
            current = Link(vals[i])
            list_of_links.append(current)
            j += 1

    for i in range(len(list_of_links)-1):
        linker(list_of_links[i], list_of_links[i+1])
