...
def balanced(m):
    ...
    True
    """
    if not (is_mobile(end(left(m)) or is_mobile(end(right(m))))):
...
def has_path(t, word):
    ...
    return label(t) == word[0] and any([has_path(b, word[1:]) for b in branches(t) if word[1:]])
    #if there's no more letters in word then complete
